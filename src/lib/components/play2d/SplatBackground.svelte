<script lang="ts">
	...
	onMount(() => {
		...
		(async () => {
			...
			let sceneScale = 5;
			// Hand-tuned starting viewpoints per world, captured with fly mode (press P).
			// Worlds not listed fall back to the auto sharpness-weighted centroid below.
			const POSE_BY_URL: Record<
				string,
				{ base: [number, number, number]; look: [number, number, number] }
			> = {
			...
			function computeViewpoint(mesh: SplatMeshType) {
				let sx = 0;
				let sy = 0;
				let sz = 0;
				let wsum = 0;
				...
				if (wsum > 0) lookCenter.set(sx / wsum, sy / wsum, sz / wsum);
				else lookCenter.set(0, 0, -1);
				...
				lookCenter.applyMatrix4(mesh.matrixWorld);
				// Degenerate (≈ surround world centred on origin) → just look forward.
				if (lookCenter.distanceTo(basePos) < 0.1) {
					lookCenter.set(basePos.x, basePos.y, basePos.z - 1);
				}
